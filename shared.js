(() => {
  ...
  if (!originalLoad) {
    // Theme and typography first; GalaxyJS background after both are ready.
    Promise.all([
      loadScript('./theme.js?v=12'),
      loadScript('./font-curator.js?v=4'),
    ]).then(() => {
      return loadScript('./vendor/galaxy.min.js?v=3.4.0');
    }).then(() => {
      return loadScript('./galaxy-background.js?v=1');
    }).catch((error) => {
      console.warn('Portfolio dynamic visual engine could not start:', error);
    });
  }
  ...
  function loadAboutResponsiveFix() {
    ...
    link.rel = 'stylesheet';
    link.href = './about-responsive-fix.css?v=2';
