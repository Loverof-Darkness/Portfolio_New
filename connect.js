(() => {
  function renderConnect() {
    ...
    style.textContent = `
      .connect-panel{min-height:100vh;height:100vh;box-sizing:border-box;padding:clamp(24px,4vh,48px) 0 40px;display:flex;flex-direction:column;justify-content:flex-start;align-items:flex-start;gap:clamp(34px,5vh,58px);overflow:auto}
      .connect-header{width:100%;max-width:950px;text-align:left;margin:0}
      ...
      .connect-header p:last-child{max-width:720px;margin:16px 0 0;color:#dce8f2;font-size:clamp(16px,1.35vw,20px);line-height:1.5;font-weight:600}
      .connect-icons{display:flex;align-items:center;gap:28px;margin-top:4px}
      ...
      @media(max-width:600px){.connect-panel{padding-top:28px}.connect-icons{gap:20px}.connect-icon{width:86px;height:86px}.connect-icon svg{width:38px;height:38px}}
    `;
    document.head.appendChild(style);
  }
  window.renderConnect = renderConnect;
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', renderConnect,{once:true}); else renderConnect();
})();
