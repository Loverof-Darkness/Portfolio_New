(() => {
  const activities = [
    { title:'Chess', kicker:'01 · STRATEGY', text:'A game of calculation, patience and long-form strategic thinking.', image:'./assets/chess-neon.svg', accent:'#00e5ff' },
    { title:'Volleyball', kicker:'02 · TEAM SPORT', text:'Fast decisions, coordination and energy on the court.', image:'./assets/volleyball-neon.svg', accent:'#00ffb3' },
    { title:'Badminton', kicker:'03 · AGILITY', text:'Speed, reflexes and focused movement — built around precision.', image:'./assets/badminton-neon.svg', accent:'#ff2bd6' },
    { title:'Computer IT & Technology', kicker:'04 · DIGITAL EXPLORATION', text:'A continuing interest in computers, software, Linux, systems and emerging technology.', image:'./assets/technology-neon.svg', accent:'#8b5cf6' }
  ];
  ...
  function renderBeyond() {
    ...
    section.innerHTML = `
      <header class="beyond-header">
        <p class="beyond-kicker">PERSONAL DIMENSION</p>
        ...
        ${activities.map((item, index) => `
          <article class="beyond-item" style="--delay:${index * 320}ms;--accent:${item.accent}">
            ...
            <div class="beyond-image-wrap">
              <img src="${item.image}" alt="Representative illustration of ${item.title}" loading="eager" />
            </div>
          </article>
        `).join('')}
      </div>
    ...
    style.textContent = `
      ...
      .beyond-kicker{margin:0 0 8px;color:#00e5ff;font:800 12px ui-monospace,monospace;letter-spacing:.24em}
      ...
      .beyond-image-wrap img{width:100%;height:100%;max-height:250px;object-fit:contain;filter:drop-shadow(0 18px 28px rgba(0,0,0,.42));transition:transform .45s ease}.beyond-item:hover .beyond-image-wrap img{transform:scale(1.04) translateY(-2px)}
